...
DEFAULT_OUTPUT_DIR = "outputs/segresnet_full_train_inference_pipeline"
DEFAULT_EXPERIMENT_NAME = "myocardium_segresnet_full_train_inference"
...
def make_training_component(args):
    ...
    return command(
        name=safe_azure_name("segresnet_full_train"),
        display_name="Myocardium SegResNet Full Training",
        description=(
            "Train one SegResNet model on all QC split folds 0-4. "
            "Fold -1 remains reserved for held-out inference."
        ),
...
def make_inference_component(args):
    ...
    return command(
        name=safe_azure_name("segresnet_fold_minus_one_inference"),
        display_name="Myocardium SegResNet Fold -1 Inference",
        description=(
            "Run inference on every fold -1 case using the single full-training "
            "checkpoint and write downloadable predicted masks."
        ),
        code=str(args.code_dir),
        command=command_text,
        environment=args.azure_environment,
...
def build_azure_pipeline(args):
    ...
    def segresnet_full_train_inference_pipeline(input_data):
        ...
        inference_node.name = safe_azure_name("segresnet_fold_minus_one_inference")
        inference_node.tags = {
            "model": "segresnet",
            "split_csv": args.split_csv,
    ...
    return segresnet_full_train_inference_pipeline(
...
def submit_pipeline(args):
    ...
    pipeline_yaml = out_dir / "segresnet_full_train_inference_pipeline_job.yml"
    ...
    submission_json = out_dir / "segresnet_full_train_inference_submission.json"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            ...
            "on folds 0-4 and queued inference on fold -1."
        )
    )
    ...
    parser.add_argument(
        ...
        default="Myocardium SegResNet Full Train + Fold -1 Inference",
    )
    parser.add_argument(
        "--azure_auth_mode",
