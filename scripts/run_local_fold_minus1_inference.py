"""Run local fold -1 inference for the single full-training SegResNet model.

The default paths match the TotalSegmentator notebook output requested for the
dice_focal full model. Heavy MONAI/PyTorch imports are kept inside the execution
path so the split/metric helpers remain testable in lightweight environments.
"""
...
DEFAULT_CHECKPOINT = (
    "notebooks/output/totalsegmentator_heart_myocardium_visualizations/"
    "models/dice_focal/best_metric_model.pth"
)
DEFAULT_OUTPUT_DIR = (
    "notebooks/output/totalsegmentator_heart_myocardium_visualizations/"
    "models/dice_focal/masks"
)
...
def _directed_distances(source: np.ndarray, target: np.ndarray, spacing: tuple[float, float, float]) -> np.ndarray:
...
def existing_case_metric_rows(output_dir: str | Path, cases: list[PatientCase]) -> list[dict]:
    output_dir = Path(output_dir)
    rows: list[dict] = []
    for case in cases:
        main_mask = output_dir / case.patient_id / "segmentation_model.nii.gz"
        metrics_path = output_dir / case.patient_id / "metrics.csv"
        if main_mask.exists() and metrics_path.exists():
            rows.extend(read_csv_rows(metrics_path))
    return rows


def write_cohort_outputs(output_dir: str | Path, cases: list[PatientCase], metric_rows: list[dict]) -> None:
    output_dir = Path(output_dir)
    metric_rows = sorted(metric_rows, key=lambda row: (row["patient_id"], row["mask_type"]))
    write_csv(output_dir / "fold_minus1_metrics.csv", metric_rows)
    write_csv(output_dir / "fold_minus1_metrics_summary.csv", summarize_metric_rows(metric_rows))
    write_csv(
        output_dir / "patient_output_folders.csv",
        [
            {
                "patient_id": case.patient_id,
                "output_dir": str(output_dir / case.patient_id),
                "mask_path": str(output_dir / case.patient_id / "segmentation_model.nii.gz"),
                "ground_truth_path": str(output_dir / case.patient_id / "ground_truth_mask.nii.gz"),
            }
            for case in cases
        ],
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run local SegResNet inference on the reserved fold -1 cases and "
            "compare predicted myocardium masks with registration_mask.nii.gz."
        )
    )
...
def _select_device(torch, requested: str):
    ...
    if requested == "mps" and not mps_available:
        raise RuntimeError(
            ...
            f"is False. torch.backends.mps.is_built()={mps_built}. Use an environment "
            "where PyTorch can see an Apple GPU, or run with --device cpu."
        )
    if requested == "cuda":
...
def run_inference(args: argparse.Namespace) -> list[dict]:
    ...
    if skipped_cases:
        ...
        write_csv(output_dir / "skipped_cases.csv", skipped_cases)
        message = (
            f"Skipping {len(skipped_cases)} unreadable case(s). "
            f"Details: {output_dir / 'skipped_cases.csv'}"
        )
        if args.fail_on_unreadable:
            raise RuntimeError(message)
        print(message, flush=True)
    else:
        stale_skipped_report = output_dir / "skipped_cases.csv"
    ...
    if args.skip_existing:
        metric_rows.extend(existing_case_metric_rows(output_dir, cases))
    ...
    if args.skip_existing:
        pending_cases = [
            case
            for case in cases
            if not (output_dir / case.patient_id / "segmentation_model.nii.gz").exists()
        ]
        skipped_count = len(cases) - len(pending_cases)
        if skipped_count:
            print(f"Skipping {skipped_count} case(s) with existing segmentation_model.nii.gz.", flush=True)

    if not pending_cases:
        write_cohort_outputs(output_dir, cases, metric_rows)
        print(f"Saved masks and metrics under: {output_dir}", flush=True)
        return metric_rows
    ...
    with torch.inference_mode():
        for case, batch in zip(pending_cases, loader):
            case_dir = output_dir / case.patient_id
            case_dir.mkdir(parents=True, exist_ok=True)
            raw_mask_path = case_dir / "segmentation_model_raw.nii.gz"
            post_mask_path = case_dir / "segmentation_model_postprocessed.nii.gz"
            main_mask_path = case_dir / "segmentation_model.nii.gz"
            image_copy_path = case_dir / "volume_CT_LATE.nii.gz"
            gt_copy_path = case_dir / "ground_truth_mask.nii.gz"
            ...
            postprocessed_mask = keep_largest_cc_numpy(raw_mask)

            save_mask_like_reference(raw_mask, case.label, raw_mask_path)
            save_mask_like_reference(postprocessed_mask, case.label, post_mask_path)
            save_mask_like_reference(postprocessed_mask, case.label, main_mask_path)
            shutil.copy2(case.image, image_copy_path)
            shutil.copy2(case.label, gt_copy_path)

            gt_mask = load_binary_nifti(case.label)
            spacing = nifti_spacing(case.label)
            case_rows = []
            for mask_type, mask, mask_path in (
                ("raw", raw_mask, raw_mask_path),
                ("postprocessed", postprocessed_mask, post_mask_path),
            ):
                row = {
                    "patient_id": case.patient_id,
                    "fold": case.fold,
                ...
                row.update(binary_mask_metrics(mask, gt_mask, spacing))
                case_rows.append(row)
                metric_rows.append(row)

            write_csv(case_dir / "metrics.csv", case_rows)
            post_row = next(row for row in case_rows if row["mask_type"] == "postprocessed")
            print(
                f"[{case.patient_id}] Dice={post_row['dice']:.4f} "
                f"IoU={post_row['iou']:.4f} "
                f"surfaceHD={post_row['surface_hausdorff_mm']:.2f}mm "
                f"HD={post_row['hausdorff_mm']:.2f}mm",
                flush=True,
            )
